...
Następujący fragment kodu przedstawia obliczenie rozkładu temperatur.
...
Korzystając z powyższej metody 
...
Posiadając model problemu tradycyjnego oraz pobranych pomiarów, zadanie polegało na odtworzeniu jednego z warunków granicznych ($h$) za pomocą równania kwadratowego:
...
\noindent $GetInverseProblemTemperatureMeasurements()$, pomocnicza metoda, która zwraca odpowiednie pomiary temperatur dla odtworzonego problemu tradycyjnego (z funkcją $h$ jako odtworzona funkcja kwadratowa), \\
...
Metoda ta jest wykorzystywana przez algorytm symulowanego wyżarzania do obliczania jak ,,najniższej" warto\si ci błędu odtworzenia i to wła\si nie algorytm przekazuje warto\si ci parametrów funkcji odtwarzanej.
...
\[ t_{ip} = rand(t_{i} - (\frac{t_{i} p}{100}), t_{i} + (\frac{t_{i} p}{100})) \]

gdzie:\\
$p$ - warto\si ć procentu zakresu błędu,\\
$t_i$ - oryginalny i-ty pomiar,\\
$t_{ip}$ - pomiar z p-procentowym zakresem błędu,\\
$rand$ - funkcja losująca liczbę z zakresu (od, do).


\subsubsection{Dokładne pomiary temperatur}

Następna tabela przedstawia wyniki osiągniete przy pomocy tych parametrów (liczby zaokrąglono do 6 miejsca po przecinku) przy niezmienionych pomiarach temperatur. \\
...
Różnicę pomiędzy pierwotną funkcją graniczną, a odtworzoną została również przedstawiona na wykresie (żółty kolor oznacza oryginał, niebieska odtworzoną, niebieska posiada większą grubo\si ć w celu zauważenia nachodzenia na siebie linii).
